...
```{r}
saveRDS(de_cellType, file="Robjects/de_cellType.rds")
...
If we change to performing differential expression analysis on the `Status` variable then there are various contrasts that can be made; `pregnancy` vs `lactation`, `lactation` vs `virgin` etc. When the `results` function is run, the table that is displayed is for the contrast `virgin vs lactation`. The `resultsNames` function can tell us which other contrasts we can access.
...
<div class="exercise">
- Use a venn diagram to visualise the overlap in the genes found to be differentially expressed in the `pregnancy vs virgin` and ` lactation vs virgin` contrasts. 
...
- Make a venn diagram for genes with adjusted p-value less than 0.05 for the comparison *virgn vs lactation* in the basal (`de_basal`) and luminal (`de_luminal`) subsets. How many genes overlap?
- How many differentially expressed genes are found with adjusted p-value less than 0.05 for the same contrast in the *multi-factor* analysis? How does this compare to the number of genes in the venn diagram? What do you think is the best way to analyse these data?
