...
The output can be converted into a data frame and manipulated in the usual manner. It is recoHsended to use `dplyr` to manipulate the data frames with the standard set of operations detailed on the [dplyr cheatsheet](https://www.rstudio.com/wp-content/uploads/2015/02/data-wrangling-cheatsheet.pdf)
...
dir.create("de_analysis",showWarnings = FALSE)
 results_treated <- results(de_treated) %>%
  as.data.frame %>% 
  rownames_to_column("ENSEMBL")  %>% 
arrange(padj)
 
  write_csv(results_treated,"de_analysis/treated_Y_vs_N_DESeq_all.csv")
```
...
```{r}
count(results_annotated, padj < 0.05)
```
...
```{r}
results(de_condition, contrast = c("condition","IR","TGF")) %>%
  as.data.frame %>% 
  rownames_to_column("ENSEMBL")  %>% 
arrange(padj) 
```
...
```{r}
anno <- AnnotationDbi::select(org.Hs.eg.db,keys=results_treated$ENSEMBL,
...
```{r}
results_annotated <- left_join(results_treated, anno,by="ENSEMBL") 
