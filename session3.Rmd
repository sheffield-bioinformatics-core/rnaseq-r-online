...
If you did not save the results from comparing `CellType`, you can use an object from the course materials
```{r}
library(DESeq2)
de_cellType <- readRDS("Robjects/de_cellType_BACKUP.rds")
results_cellType <- results(de_cellType) %>% 
  as.data.frame %>% 
  tibble::rownames_to_column("ENSEMBL")
```
...
Error: package or namespace load failed for ‘org.Mm.eg.db’: .onLoad failed in loadNamespace() for 'org.Mm.eg.db', details: call: l$contains error: $ operator is invalid for atomic vectors
```
...
```{r}
gene_labels <- results_annotated$SYMBOL[1:N]
...
- Load your `de_status.rds` file. If you didn't create this, there is a backup object provided 
```{r eval=FALSE}
## Either
de_status <- readRDS("Robjects/de_status.rds")
##OR
de_status <- readRDS("Robjects/de_status_BACKUP.rds")
...
- What does the `scale` argument do in `pheatmap`. Why might it be useful to change the argument?
...
- Suppose we have a zebrafish dataset rather than human or mouse. What `org...db` package would you use in Bioconductor? How would you connect to the zebrafish database in biomaRt?
