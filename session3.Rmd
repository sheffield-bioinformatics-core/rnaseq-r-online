...
```{r}
N <- 10
top_genes <- slice(results_tgf, 1:N) %>% pull(ENSEMBL)
...
library(pheatmap)
top_genes <- slice(results_tgf, 1:10) %>% pull(ENSEMBL)
vsd <- vst(dds)
...
```{r}
gene_labels <- slice(results_tgf, 1:N) %>% pull(SYMBOL)
...
```{r}
anno_df <- dplyr::select(anno,ENSEMBL,SYMBOL)  %>% 
