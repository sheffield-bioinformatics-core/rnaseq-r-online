...
plotPCA(vsd,intgroup="Treated",returnData = TRUE) %>% 
  dplyr::rename(Run = name) %>% 
  left_join(sampleinfo) %>% 
  dplyr::rename(Batch = condition) %>% 
  dplyr::mutate(condition = Replicate) %>% 
  mutate(Batch = forcats::fct_recode(Batch, "1" = "CTR", "2" = "TGF","3"= "IR")) %>% 
  mutate(condition = forcats::fct_recode(as.factor(condition), "CTR"="1", "TGF"="2","IR"="3")) %>% 
  ggplot(aes(x = PC1, y = PC2,col=Batch,label=condition)) + geom_point() + geom_text_repel(col="black") + ggtitle("Worrying PCA Example")
```
