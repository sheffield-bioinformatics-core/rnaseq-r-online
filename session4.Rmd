...
If you did not save the results from annotation the cellType results, you can use an object from the course materials
```{r}
results_annotated <- readRDS("Robjects/results_cellType_annotated_BACKUP.rds")
```
...
<div class="exercise">
- Use the `AnnotationDbi::select` function to find `ENSEMBL` for the GO term `GO:0016020`?
...
<div class="exercise">
- Load your annotated DESeq2 results comparing Virgin to lactation samples
```{r eval=FALSE}
## Either
results_annotated <- readRDS("Robjects/results_status_annotated.rds")
##OR
results_annotated_status <- readRDS("Robjects/results_status_annotated_BACKUP.rds")

```
- Use `clusterProfiler` to identify the most-enriched pathways in the comparison and visualise the results
- 
